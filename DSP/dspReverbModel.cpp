...
*	@date		13-Jun-2026
*	@version	1.3 
*					1. input signal splitted into L and R instad of (L+R)/2. 
*					
*	@History	2-Oct-2024	1.2	Code refactoring and notaion.
*				25_Jan-2021	1.1	Code refactoring and notaion. 
...
{
	float out_L, out_R, in_L, in_R;//, input;
	...
	while (num_samples-- > 0)
	{
		...
		for (int i = 0; i < num_combs; i++)
		{
			...
			out_L += comb_L[i].process(in_L);	
			out_R += comb_R[i].process(in_R);
		}

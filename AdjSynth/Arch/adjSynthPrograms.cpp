...
AdjSynthPrograms::~AdjSynthPrograms()
{
	...
	for (auto it = assigned_voices_vector.begin(); it != assigned_voices_vector.end(); ++it)
	{
		if (*it && (assigned_voices_vector.size() > 0))
		{
			deallocate_voice_from_program(**it);
		}
	}
	assigned_voices_vector.clear();
...
void AdjSynthPrograms::set_program_preset_params(_settings_params_t *preset_params)
{
	...
	for (auto it = assigned_voices_vector.begin(); it != assigned_voices_vector.end(); ++it)
	{
		if (*it && (assigned_voices_vector.size() > 0))
		{
			SynthVoice *voice = AdjSynth::get_instance()->synth_voice[**it];
...
int AdjSynthPrograms::assign_voice_with_preset_program_params(SynthVoice *voice, int voice_num)
{
	...
	//int *vnum = new int;
	auto vnum = std::make_unique<int>(voice_num);
	//*vnum = voice_num;
	assigned_voices_vector.push_back(std::move(vnum));
...
int AdjSynthPrograms::	deallocate_voice_from_program(int voice_num)
{
	...
	for (auto it = assigned_voices_vector.begin(); it != assigned_voices_vector.end();)
	{
		if (*it && (assigned_voices_vector.size() > 0))
		{
			if (**it == voice_num)
			{
				...
				// Erase the element. The erase() function returns a valid iterator
				// to the element following the one that was erased.
				it = assigned_voices_vector.erase(it);
				res = 0;
			}
			else if (!AdjSynth::get_instance()->synth_voice[**it]->audio_voice->is_voice_wait_for_not_active() &&
					 !AdjSynth::get_instance()->synth_voice[**it]->audio_voice->is_voice_active())
			{
...
int AdjSynthPrograms::get_voice_num_playing_note(int note)
{
	...
	printf("get_voice_num_playing_note entry num of voices %i \n", (int)assigned_voices_vector.size());
	for (auto it = assigned_voices_vector.begin(); it != assigned_voices_vector.end();)
	{
		if (*it && (assigned_voices_vector.size() > 0))
		{
			voice = AdjSynth::get_instance()->synth_voice[**it];
			if (voice != NULL)
			{
	...
	printf("get_voice_num_playing_note exit num of voices %i \n", (int)assigned_voices_vector.size());
...
int AdjSynthPrograms::set_program_voices_poly_mixer_gain_1_level_int(int level)
{
	...
	for (auto it = assigned_voices_vector.begin(); it != assigned_voices_vector.end(); ++it)
	{
		if (*it && (assigned_voices_vector.size() > 0))
		{
			parent_audio_poly_mixer->set_voice_gain1_level(**it, // voice num
...
int AdjSynthPrograms::set_program_voices_poly_mixer_gain_2_level_int(int level)
{
	...
	for (auto it = assigned_voices_vector.begin(); it != assigned_voices_vector.end(); ++it)
	{
		if (*it && (assigned_voices_vector.size() > 0))
		{
			parent_audio_poly_mixer->set_voice_gain2_level(**it, // voice num
...
int AdjSynthPrograms::set_program_voices_poly_mixer_pan_1_int(int pan)
{
	...
	for (auto it = assigned_voices_vector.begin(); it != assigned_voices_vector.end(); ++it)
	{
		if (*it && (assigned_voices_vector.size() > 0))
		{
			parent_audio_poly_mixer->set_voice_pan1(**it, // voice num
...
int AdjSynthPrograms::set_program_voices_poly_mixer_pan_2_int(int pan)
{
	...
	for (auto it = assigned_voices_vector.begin(); it != assigned_voices_vector.end(); ++it)
	{
		if (*it && (assigned_voices_vector.size() > 0))
		{
			parent_audio_poly_mixer->set_voice_pan2(**it, // voice num
...
int AdjSynthPrograms::set_program_voices_poly_mixer_send_1_int(int send)
{
	...
	for (auto it = assigned_voices_vector.begin(); it != assigned_voices_vector.end(); ++it)
	{
		if (*it && (assigned_voices_vector.size() > 0))
		{
			parent_audio_poly_mixer->set_voice_send1_level(**it, // voice num
...
int AdjSynthPrograms::set_program_voices_poly_mixer_send_2_int(int send)
{
	...
	for (auto it = assigned_voices_vector.begin(); it != assigned_voices_vector.end(); ++it)
	{
		if (*it && (assigned_voices_vector.size() > 0))
		{
			parent_audio_poly_mixer->set_voice_send2_level(**it, // voice num

...
void SynthVoice::set_voice_params(_settings_params_t *params)
{
	...
	res = settings_manager->get_int_param(params, "adjsynth.poly_mixer.level", &int_param);
	if (res == _SETTINGS_KEY_FOUND)
	{
		AdjSynth::get_instance()->audio_polyphony_mixer->set_voice_level(voice_num, (float)int_param.value / 100.0);
	}
	
	res = settings_manager->get_int_param(params, "adjsynth.poly_mixer.pan", &int_param);
	if (res == _SETTINGS_KEY_FOUND)
	{
		AdjSynth::get_instance()->audio_polyphony_mixer->set_voice_pan(voice_num, (float)(int_param.value - 50) / 50.0); // -1.0 to 1.0
	}
	
	res = settings_manager->get_int_param(params, "adjsynth.poly_mixer.send", &int_param);
	if (res == _SETTINGS_KEY_FOUND)
	{
		AdjSynth::get_instance()->audio_polyphony_mixer->set_voice_send(voice_num, (float)int_param.value / 100.0); 
	}
	
	res = settings_manager->get_int_param(params, "adjsynth.poly_mixer.pan_lfo_num", &int_param);
	if (res == _SETTINGS_KEY_FOUND)
	{
		AdjSynth::get_instance()->audio_polyphony_mixer->set_voice_pan_lfo(voice_num, int_param.value);
	}
	
	res = settings_manager->get_int_param(params, "adjsynth.poly_mixer.pan_lfo_level", &int_param);
	if (res == _SETTINGS_KEY_FOUND)
	{
